            ...
            CallExpression(node) {
                ...
                if (callee.type === 'MemberExpression' && resourceMethods.has(callee.property.name)) {
                    ...
                    if (!hasExistenceCheck) {
                        context.report({
                            node: node,
                            message: 'External resource method called without checking existence. For more information, visit https://github.com/luissmonteiro/eslint-plugin-test-smells/blob/main/docs/rules/resource-optimism.md'
                        });
                    }
                }
            }
