                ...
                if (node.callee.name === 'test' || node.callee.name === 'it') {
                    ...
                    if (testFunction && testFunction.type === 'ArrowFunctionExpression') {
                        ...
                        if (methodNames.size > 2) {
                            context.report({
                                node: node,
                                message: 'Avoid calling multiple production methods in a single test. For more information, visit https://github.com/luissmonteiro/eslint-plugin-test-smells/blob/main/docs/rules/eager-test.md',
                            });
                        }
                    }
                }
